...
def generate_img_and_mask(height, width):
    ...
    # Create target masks
    masks = np.asarray([
        add_filled_square(np.zeros(shape, dtype=bool), *square_location) * 10,
        add_circle(np.zeros(shape, dtype=bool), *circle_location2, fill=True) * 20,
        add_triangle(np.zeros(shape, dtype=bool), *triangle_location) * 30,
        add_circle(np.zeros(shape, dtype=bool), *circle_location1) * 40,
        add_filled_square(np.zeros(shape, dtype=bool), *mesh_location) * 50,
        # add_mesh_square(np.zeros(shape, dtype=bool), *mesh_location),
        add_plus(np.zeros(shape, dtype=bool), *plus_location) * 60
    ]).astype(np.float32)
